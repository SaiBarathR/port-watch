...
fn current_username() -> String {
    std::env::var("USER").unwrap_or_else(|_| "ginpachi".to_string())
}
...
mod tests {
    ...
    fn classifies_user_process() {
        ...
        let mut p = sample_process(
            ...
            &current_username(),
            &format!("{home}/hermes-projects/hosted-web-projects"),
        );
        classify(&mut p);
    ...
    #[test]
    fn classifies_root_httpd_as_apple() {
        let mut p = sample_process("/usr/sbin/httpd", "root", "/");
        classify(&mut p);
        assert_eq!(p.system_kind, SystemKind::Apple);
    }
}
